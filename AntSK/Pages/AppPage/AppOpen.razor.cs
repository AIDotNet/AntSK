...
namespace AntSK.Pages.AppPage
{
    public partial class AppOpen
    {
        ...
        {
            _desc = @$"为了方便其他应用对接，接口符合openai规范。{Environment.NewLine}BaseUrl:{Environment.NewLine}{_openApiUrl} {Environment.NewLine}headers:{Environment.NewLine}Authorization: ""Bearer {_appModel.SecretKey}"" {Environment.NewLine}Body:  {Environment.NewLine}{JsonConvert.SerializeObject(new OpenAIModel(), Formatting.Indented)}";
        }
